...
async def serve(q: Q):
    ...
    if not q.client.initialized:
        ...
        logger.debug(f"File to upload: {q.args.file_upload[0]}")
...
async def init(q: Q) -> None:
    ...
    q.app.load_image, = await q.site.upload(['load.gif'])
    ...
    q.page["header"] = ui.header_card(
        ...
        nav = [
            ...
            ui.nav_group(
                label='Settings',
                items=[
                    ui.nav_item(name='nav_home', label='Info', icon='Info'),
                    ui.nav_item(name='nav_home', label='Settings', icon='Settings'),
                ]
            )
        ]
    )
...
@on()
async def nav_browse(q: Q):
    clear_cards(q)
    q.page["navigation"].value = "nav_browse"
    ...
    header = ui.text_l("")
    q.page["browse"] = ui.form_card(
        box="main",
        items=[
              # ui.buttons(justify='end', items=[
              #       ui.button(name='browse_dataset', label='Browse'),
              #       ui.button(name='sample_dataset', label='Import Demo Data', primary=True),
              # ]),
               ui.file_upload(
                    name="file_upload",
                    ...
                    height='400px',
                    width='400px',
                   tooltip='Score file must contain the fields: File_ID, Event_Date, Event_Description, h2oGPTe_Response'
                    #max_file_size=50,
               ),
               header,
              ],
    )
    q.client.cards.append("browse")
...
async def nav_import(q: Q):
    ...
    if q.client.original_data is not None:
        ...
        header = ui.text_l("Safety Event Upload/Preview")
        table = ui.table(
            name="preview_data",
            columns=[
...
async def edit_data(q: Q):
    ...
    q.page["edit_data1"] = ui.form_card(
        ...
        items=[ui.text_xl('Human-in-the-loop - Edit or Generate New Response'),
               ...
               ui.text_xl(' '),
               ui.text_xl(' '),
               ui.text_l('Generate a New Classification Response'),
               ...
               ui.buttons(justify='end', items=[
                   ui.button(name="get_catalog_prompt", label="Select Prompt from Catalog"),
                   ui.button(name="generate_response", label="Generate New Response")]),
...
async def edit_email(q: Q):
                ...
                zones=[
                    ...
                    ui.zone(
                        ...
                        zones=[
                            ...
                            ui.zone(name="main", direction=ui.ZoneDirection.ROW,
                                    zones=[ui.zone(name="main_0", size="50%"),
                                           ui.zone(name="main_1", size="50%", direction=ui.ZoneDirection.COLUMN,
                                                   zones = [ui.zone(name='main_1_a', size='88%'),
                                                            ui.zone(name='main_1_b', size='12%')])
                                           ]
                                    ),
                        ],
                    ),
    ...
    q.page["edit_email1"] = ui.form_card(
        box="main_1_a",
    ...
    q.page["edit_email2"] = ui.form_card(
    box = "main_1_b",
    items = [
             ui.buttons(justify='end', items=[
                 ui.button(name="cancel_email_edits", label="Cancel and Return to Preview"),
                 ui.button(name="save_email_edits", label="Save Edited Email"),
                 ui.button(name="send_email_edits", label="Send", primary=True),
             ])
             ]
    )

    q.client.cards.append("edit_email1")
    q.client.cards.append("edit_email2")
...
async def edit_prompt(q: Q):
    ...
    q.page["edit_prompt2"] = ui.form_card(
    box = "main_1_b",
    items = [
             ui.buttons(justify='end', items=[
                 ui.button(name="cancel_prompt_edits", label="Cancel and Return to Prompt Catalog"),
                 ui.button(name="save_prompt_edits", label="Save as New Prompt", primary=True),
             ])
             ]
    )
    q.client.cards.append("edit_prompt")
    q.client.cards.append("edit_prompt2")
